...
_PHAGE_KEYWORDS = {
    'phage', 'bacteriophage', 'virus', 'viral', 'capsid', 'tail', 'terminase', 
    'baseplate', 'prophage', 'virion', 'lysin', 'holin', 'integrase'
}

_ENZYME_KEYWORDS = {
    'enzyme', 'catalytic', 'hydrolase', 'transferase', 'oxidoreductase', 'ligase',
    'isomerase', 'polymerase', 'synthetase', 'kinase', 'phosphatase', 'protease',
    'reductase', 'dehydrogenase', 'oxydase'
}
...
def choose_bucket(a: str, b: str) -> str:
    ...
    try:
        ...
        if _RE_EC_NUMBER.search(joined):
            logger.debug(f"Bucket selected: enzyme (EC number match)")
            return "v1.1-enzyme"
            
        # Specific regex patterns (stronger signals)
        if _RE_PHAGE.search(joined):
            logger.debug(f"Bucket selected: phage (regex match)")
            return "v1.1-phage"
            
        if _RE_ENZYME.search(joined):
            logger.debug(f"Bucket selected: enzyme (regex match)")
            return "v1.1-enzyme"
            
        # Keyword-based routing (weaker signals)
        if _text_contains_keywords(joined, _PHAGE_KEYWORDS):
            logger.debug(f"Bucket selected: phage (keyword match)")
            return "v1.1-phage"
            
        if _text_contains_keywords(joined, _ENZYME_KEYWORDS):
            logger.debug(f"Bucket selected: enzyme (keyword match)")
            return "v1.1-enzyme"
            
        # Default bucket
        logger.debug(f"Bucket selected: general (default)")
        return "v1.1-general"
    except Exception as e:
...
# Template helpers
# -------------------------------------------------------------------
def get_prompt_template(cfg: Dict[str, Any],
                        *,
                        ver: str | None = None,
                        bucket_pair: tuple[str, str] | None = None) -> str:
    """
    ...
    try:
        ...
        if key.startswith("v1.1-"):
            ...
            if bucket_prompt_class and hasattr(bucket_prompt_class, '_mock_responses') and key in bucket_prompt_class._mock_responses:
